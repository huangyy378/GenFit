...
EventDisplay::EventDisplay()
    : errorScale_(1.), drawGeometry_(false), drawDetectors_(true),
      drawHits_(true), drawErrors_(true), drawPlanes_(true),
      drawTrackMarkers_(true), drawTrack_(true), drawRefTrack_(true),
      drawForward_(true), drawBackward_(true), drawAutoScale_(true),
      drawScaleMan_(false), drawSilent_(false), drawCardinalRep_(true),
      repId_(0), drawAllTracks_(true), trackId_(0), refit_(false), debugLvl_(0),
      fitterId_(SimpleKalman), mmHandling_(weightedAverage),
      squareRootFormalism_(false), dPVal_(1.E-3), dRelChi2_(0.2), dChi2Ref_(1.),
      nMinIter_(2), nMaxIter_(4), nMaxFailed_(-1), resort_(false) {

  if ((!gApplication) ||
      (gApplication &&
       gApplication->TestBit(TApplication::kDefaultApplication))) {
    std::cout << "In EventDisplay ctor: gApplication not found, creating..."
              << std::flush;
    new TApplication("ROOT_application", 0, 0);
    std::cout << "done!" << std::endl;
  }
  if (!gEve) {
    std::cout << "In EventDisplay ctor: gEve not found, creating..."
              << std::flush;
    TEveManager::Create();
    std::cout << "done!" << std::endl;
  }
...
void EventDisplay::next(unsigned int stp) { gotoEvent(eventId_ + stp); }
...
void EventDisplay::gotoEvent(unsigned int id) {
  ...
  if (old_error_scale != errorScale_) {
    ...
    drawEvent(eventId_,
              resetCam); // if autoscaling changed the error, draw again.
  }
...
void EventDisplay::open() {
  ...
  std::cout << "EventDisplay::open(); " << getNEvents() << " events loaded"
            << std::endl;
...
void EventDisplay::drawEvent(unsigned int id, bool resetCam) {
  ...
  // draw the geometry, does not really work yet. If it's fixed, the docu in the
  // header file should be changed.
  if (drawGeometry_) {
  ...
  for (unsigned int i = 0; i < events_.at(id)->size();
       i++) { // loop over all tracks in an event
    ...
    if (refit_) {
      ...
      case SimpleKalman:
        ...
        (static_cast<KalmanFitter *>(fitter.get()))
            ->useSquareRootFormalism(squareRootFormalism_);
        break;

      case RefKalman:
        fitter.reset(new KalmanFitterRefTrack(nMaxIter_, dPVal_));
        fitter->setMultipleMeasurementHandling(mmHandling_);
        static_cast<KalmanFitterRefTrack *>(fitter.get())
            ->setDeltaChi2Ref(dChi2Ref_);
        break;

      case DafSimple:
        fitter.reset(new DAF(false));
        (static_cast<KalmanFitter *>(
             (static_cast<DAF *>(fitter.get()))->getKalman()))
            ->useSquareRootFormalism(squareRootFormalism_);
        break;
      case DafRef:
      ...
      int microseconds = 1000000 * (endcputime.tv_sec - startcputime.tv_sec) +
                         (endcputime.tv_usec - startcputime.tv_usec);
      std::cout << "it took " << double(microseconds) / 1000
    ...
    for (unsigned int j = 0; j < numhits;
         j++) { // loop over all hits in the track
      ...
      if (tp->getNumRawMeasurements() > 1) {
        ...
        if (!sameTypes) {
          std::cerr << "cannot draw trackpoint containing multiple "
                       "Measurements of differend types"
                    << std::endl;
          continue;
        }
      }
      ...
      bool space_hit =
          (dynamic_cast<const SpacepointMeasurement *>(m) != nullptr);
      bool helix_hit = (dynamic_cast<const HelixMeasurement *>(m) != nullptr);
      bool wire_hit = m && m->isLeftRightMeasurement();
      bool wirepoint_hit =
          wire_hit &&
          (dynamic_cast<const WirePointMeasurement *>(m) != nullptr);
      if (!full_hit && !planar_hit && !planar_pixel_hit && !space_hit &&
          !wire_hit && !wirepoint_hit && !helix_hit) {
        std::cout << "Track " << i << ", Hit " << j
      ...
      for (unsigned int iMeas = 0; iMeas < nMeas; ++iMeas) {
        ...
        if (planar_hit) {
          ...
          hit_v =
              v *
              (track_pos -
               o); // move the covariance tube so that the track goes through it
          if (wirepoint_hit) {
            hit_v = hit_coords(1);
          }
        } else if (helix_hit) {
        ...
        // -----------------------------------------
        if (iMeas == 0 && (drawPlanes_ || (drawDetectors_ && planar_hit))) {
          TVector3 move(0, 0, 0);
          if (planar_hit)
            move = track_pos - o;
          if (wire_hit)
            move = v * (v * (track_pos - o)); // move the plane along the wire
                                              // until the track goes through it
          TEveBox *box =
        ...
        try {
          if (j == 0) {
            if (drawBackward_) {
              ...
              makeLines(&update, fi->getBackwardUpdate(), rep, kMagenta, 1,
                        drawTrackMarkers_, drawErrors_, 1);
            }
          }
          if (j > 0 && prevFi != nullptr) {
            if (drawTrack_) {
              makeLines(prevFittedState, fittedState, rep,
                        charge > 0 ? kRed : kBlue, 1, drawTrackMarkers_,
                        drawErrors_, 3);
              if (drawErrors_) { // make sure to draw errors in both directions
                makeLines(prevFittedState, fittedState, rep,
                          charge > 0 ? kRed : kBlue, 1, false, drawErrors_, 0,
                          0);
              }
            }
            if (drawForward_) {
              makeLines(prevFi->getForwardUpdate(), fi->getForwardPrediction(),
                        rep, kCyan, 1, drawTrackMarkers_, drawErrors_, 1, 0);
              if (j == numhits - 1) {
                MeasuredStateOnPlane update(*fi->getForwardUpdate());
                update.extrapolateBy(3.);
                makeLines(fi->getForwardUpdate(), &update, rep, kCyan, 1,
                          drawTrackMarkers_, drawErrors_, 1, 0);
              }
            }
            if (drawBackward_) {
              makeLines(prevFi->getBackwardPrediction(),
                        fi->getBackwardUpdate(), rep, kMagenta, 1,
                        drawTrackMarkers_, drawErrors_, 1);
            }
            // draw reference track if corresponding option is set
            // ------------------------------------------
            if (drawRefTrack_ && fi->hasReferenceState() &&
                prevFi->hasReferenceState())
              makeLines(prevFi->getReferenceState(), fi->getReferenceState(),
                        rep, charge > 0 ? kRed + 2 : kBlue + 2, 2,
                        drawTrackMarkers_, false, 3);
        ...
        if (drawDetectors_) {
          ...
          if (wire_hit) {
            ...
            det_shape->SetShape(
                new TGeoTube(std::max(0., (double)(hit_u - 0.0105 / 2.)),
                             hit_u + 0.0105 / 2., plane_size));

            TVector3 norm = u.Cross(v);
            TGeoRotation det_rot(
                "det_rot", (u.Theta() * 180) / TMath::Pi(),
                (u.Phi() * 180) / TMath::Pi(),
                (norm.Theta() * 180) / TMath::Pi(),
                (norm.Phi() * 180) / TMath::Pi(),
                (v.Theta() * 180) / TMath::Pi(),
                (v.Phi() * 180) / TMath::Pi()); // move the tube to the right
                                                // place and rotate it correctly
            TVector3 move =
                v * (v * (track_pos - o)); // move the tube along the wire until
                                           // the track goes through it
            TGeoCombiTrans det_trans(o(0) + move.X(), o(1) + move.Y(),
                                     o(2) + move.Z(), &det_rot);
        ...
        if (drawHits_) {
          ...
          if (full_hit) {
            ...
            StateOnPlane dummy2(
                TVectorD(rep->getDim()),
                static_cast<const FullMeasurement *>(m)->constructPlane(dummy),
                rep);
            MeasuredStateOnPlane sop =
                *(static_cast<const FullMeasurement *>(m)
                      ->constructMeasurementsOnPlane(dummy2)[0]);
          ...
          if (planar_hit) {
            if (!planar_pixel_hit) {
              ...
              hit_box = boxCreator(
                  (o + move + hit_u * stripDir3), stripDir3, stripDir3perp,
              ...
              if (drawAutoScale_) {
                ...
                if (min_cov < 1e-5) {
                  std::cout << "Track " << i << ", Hit " << j
                            << ": Invalid covariance matrix (Eigenvalue < "
                               "1e-5), autoscaling not possible!"
                            << std::endl;
                } else {
                  if (min_cov < 0.049) {
                    double cor = 0.05 / min_cov;
                    std::cout << "Track " << i << ", Hit " << j
                              << ": Pixel covariance too small, rescaling by "
                              << cor;
                    errorScale_ *= cor;
                    pseudo_res_0 *= cor;
                    pseudo_res_1 *= cor;
              ...
              TVector3 u_semiaxis =
                  (pix_pos + eVec(0, 0) * u + eVec(1, 0) * v) - pix_pos;
              TVector3 v_semiaxis =
                  (pix_pos + eVec(0, 1) * u + eVec(1, 1) * v) - pix_pos;
              TVector3 norm = u.Cross(v);
              ...
              TGeoRotation det_rot("det_rot",
                                   (u_semiaxis.Theta() * 180) / TMath::Pi(),
                                   (u_semiaxis.Phi() * 180) / TMath::Pi(),
                                   (v_semiaxis.Theta() * 180) / TMath::Pi(),
                                   (v_semiaxis.Phi() * 180) / TMath::Pi(),
                                   (norm.Theta() * 180) / TMath::Pi(),
                                   (norm.Phi() * 180) / TMath::Pi());
              TGeoCombiTrans det_trans(pix_pos(0), pix_pos(1), pix_pos(2),
                                       &det_rot);
          ...
          if (space_hit) {
            {
              ...
              TGeoRotation det_rot("det_rot", eVec1.Theta() * radDeg,
                                   eVec1.Phi() * radDeg, eVec2.Theta() * radDeg,
                                   eVec2.Phi() * radDeg, eVec3.Theta() * radDeg,
                                   eVec3.Phi() * radDeg);
              ...
              // -----------------------------------------------------
              if (drawAutoScale_) {
                double min_cov = std::min(pseudo_res_0,
                ...
                if (min_cov < 1e-5) {
                  ...
                  if (min_cov <= 0.149) {
                    ...
                    std::cout
                        << "Track " << i << ", Hit " << j
                        << ": Space hit covariance too small, rescaling by "
                        << cor;
                    errorScale_ *= cor;
                    pseudo_res_0 *= cor;
                    pseudo_res_1 *= cor;
              ...
              // -------------------------------------------------------
              TGeoGenTrans det_trans(
                  o(0), o(1), o(2),
                  // std::sqrt(pseudo_res_0/pseudo_res_1/pseudo_res_2),
                  // std::sqrt(pseudo_res_1/pseudo_res_0/pseudo_res_2),
                  // std::sqrt(pseudo_res_2/pseudo_res_0/pseudo_res_1), // this
                  // workaround is necessary due to the "normalization"
                  // performed in  TGeoGenTrans::SetScale
                  // 1/(pseudo_res_0),1/(pseudo_res_1),1/(pseudo_res_2),
                  pseudo_res_0, pseudo_res_1, pseudo_res_2, &det_rot);
              cov_shape->SetTransMatrix(det_trans);
              // finished rotating and translating
              // ------------------------------------------
...

            {
              ...
              // ------------------------------
              static const double radDeg(180. / TMath::Pi());
              TGeoRotation det_rot("det_rot", u_semiaxis.Theta() * radDeg,
                                   u_semiaxis.Phi() * radDeg,
                                   v_semiaxis.Theta() * radDeg,
                                   v_semiaxis.Phi() * radDeg,
          ...
          // draw helix hits ---------------
          if (helix_hit) {
            const TVectorD &rawHitCoords = m->getRawHitCoords(); // get params

            if (drawDetectors_) {
              // draw helix
              TEveStraightLineSet *helix_line =
                  new TEveStraightLineSet("helix_line");
              helix_line->SetLineColor(kCyan);
              helix_line->SetLineWidth(20);

              const TVector3 center(rawHitCoords(0), rawHitCoords(1),
                                    rawHitCoords(2));
              const double radius = rawHitCoords(3);
              const double pitchAngle = rawHitCoords(4);
              const double phi0 = rawHitCoords(5);
              const double phiTotal = rawHitCoords(6);
              ...
              for (int k = 0; k <= nPoints; ++k) {
                const double u1 = k * phiTotal / nPoints;
                const double rotatedU = u1 + phi0;
                TVector3 helixPt(radius * std::cos(rotatedU),
                                 radius * std::sin(rotatedU),
                                 u1 * tanAR - zOffset);
                helixPt += center;

                if (!first) {
                  helix_line->AddLine(prev_pt(0), prev_pt(1), prev_pt(2),
                                      helixPt(0), helixPt(1), helixPt(2));
                }
                prev_pt = helixPt;
                first = false;
              }
              gEve->AddElement(helix_line);
            }
          }
          // finished drawing helix hits
          // ------------------------------------------------
          ...
          if (wire_hit) {
            ...
            if (wirepoint_hit)
              pseudo_res_1 = errorScale_ * std::sqrt(hit_cov(1, 1));
            ...
            if (drawAutoScale_) {
              if (pseudo_res_0 < 1e-5) {
                std::cout << "Track " << i << ", Hit " << j
                          << ": Invalid wire resolution (< 1e-5), autoscaling "
                             "not possible!"
                          << std::endl;
              } else {
                if (pseudo_res_0 < 0.0049) {
                  ...
                  std::cout << "Track " << i << ", Hit " << j
                            << ": Wire covariance too small, rescaling by "
                            << cor;
                  errorScale_ *= cor;
                  pseudo_res_0 *= cor;
                  std::cout << " to " << errorScale_ << std::endl;
            ...
            hit_box = boxCreator((o + move + hit_u * u), u, v,
...
TEveBox *EventDisplay::boxCreator(TVector3 o, TVector3 u, TVector3 v, float ud,
  ...
  for (int k = 0; k < 24; k += 3)
    box->SetVertex((k / 3), vertices[k], vertices[k + 1], vertices[k + 2]);

  return box;
}

void EventDisplay::makeLines(const StateOnPlane *prevState,
                             const StateOnPlane *state, const AbsTrackRep *rep,
                             const Color_t &color, const Style_t &style,
                             bool drawMarkers, bool drawErrors,
  ...
  lineSet->AddLine(oldPos(0), oldPos(1), oldPos(2), intermediate1(0),
                   intermediate1(1), intermediate1(2));
  lineSet->AddLine(intermediate1(0), intermediate1(1), intermediate1(2),
                   intermediate2(0), intermediate2(1), intermediate2(2));
  lineSet->AddLine(intermediate2(0), intermediate2(1), intermediate2(2), pos(0),
                   pos(1), pos(2));
  ...
  if (drawErrors) {
    ...
    if (measuredState != nullptr) {
      ...
      TEveTriangleSet *error_shape =
          new TEveTriangleSet(vertices.size(), nEdges * 2);
      for (unsigned int k = 0; k < vertices.size(); ++k) {
        error_shape->SetVertex(k, vertices[k].X(), vertices[k].Y(),
                               vertices[k].Z());
      }
      ...
      int iTri(0);
      for (int i = 0; i < nEdges; ++i) {
        // error_shape->SetTriangle(iTri++,  0,             i+1,
        // (i+1)%nEdges+1);
        error_shape->SetTriangle(iTri++, i + 1, i + 1 + nEdges,
                                 (i + 1) % nEdges + 1);
        error_shape->SetTriangle(iTri++, (i + 1) % nEdges + 1, i + 1 + nEdges,
                                 (i + 1) % nEdges + 1 + nEdges);
        // error_shape->SetTriangle(iTri++,  2*nEdges+1,    i+1+nEdges,
        // (i+1)%nEdges+1+nEdges);
      }
...
void EventDisplay::makeGui() {
  ...
  TGHorizontalFrame *hf = new TGHorizontalFrame(frmMain);
  {
    ...
    guiEvent->Connect("ValueSet(Long_t)", "genfit::EventDisplay", fh,
                      "guiGoto()");
  ...
  hf = new TGHorizontalFrame(frmMain);
  {
    ...
    if (drawGeometry_)
      guiDrawGeometry_->Toggle();
    hf->AddFrame(guiDrawGeometry_);
    guiDrawGeometry_->Connect("Toggled(Bool_t)", "genfit::EventDisplay", fh,
                              "guiSetDrawParams()");
  }
  frmMain->AddFrame(hf);
  ...
  hf = new TGHorizontalFrame(frmMain);
  {
    ...
    if (drawDetectors_)
      guiDrawDetectors_->Toggle();
    hf->AddFrame(guiDrawDetectors_);
    guiDrawDetectors_->Connect("Toggled(Bool_t)", "genfit::EventDisplay", fh,
                               "guiSetDrawParams()");
  }
  frmMain->AddFrame(hf);
  ...
  hf = new TGHorizontalFrame(frmMain);
  {
    ...
    guiDrawHits_->Connect("Toggled(Bool_t)", "genfit::EventDisplay", fh,
                          "guiSetDrawParams()");
  }
  frmMain->AddFrame(hf);
  ...
  hf = new TGHorizontalFrame(frmMain);
  {
    ...
    if (drawPlanes_)
      guiDrawPlanes_->Toggle();
    hf->AddFrame(guiDrawPlanes_);
    guiDrawPlanes_->Connect("Toggled(Bool_t)", "genfit::EventDisplay", fh,
                            "guiSetDrawParams()");
  }
  frmMain->AddFrame(hf);
  ...
  hf = new TGHorizontalFrame(frmMain);
  {
    ...
    if (drawTrackMarkers_)
      guiDrawTrackMarkers_->Toggle();
    hf->AddFrame(guiDrawTrackMarkers_);
    guiDrawTrackMarkers_->Connect("Toggled(Bool_t)", "genfit::EventDisplay", fh,
                                  "guiSetDrawParams()");
  }
  frmMain->AddFrame(hf);
  ...
  hf = new TGHorizontalFrame(frmMain);
  {
    ...
    if (drawTrack_)
      guiDrawTrack_->Toggle();
    hf->AddFrame(guiDrawTrack_);
    guiDrawTrack_->Connect("Toggled(Bool_t)", "genfit::EventDisplay", fh,
                           "guiSetDrawParams()");
  }
  frmMain->AddFrame(hf);
  ...
  hf = new TGHorizontalFrame(frmMain);
  {
    ...
    if (drawRefTrack_)
      guiDrawRefTrack_->Toggle();
    hf->AddFrame(guiDrawRefTrack_);
    guiDrawRefTrack_->Connect("Toggled(Bool_t)", "genfit::EventDisplay", fh,
                              "guiSetDrawParams()");
  }
  frmMain->AddFrame(hf);
  ...
  hf = new TGHorizontalFrame(frmMain);
  {
    ...
    if (drawErrors_)
      guiDrawErrors_->Toggle();
    hf->AddFrame(guiDrawErrors_);
    guiDrawErrors_->Connect("Toggled(Bool_t)", "genfit::EventDisplay", fh,
                            "guiSetDrawParams()");
  }
  frmMain->AddFrame(hf);
  ...
  hf = new TGHorizontalFrame(frmMain);
  {
    ...
    if (drawForward_)
      guiDrawForward_->Toggle();
    hf->AddFrame(guiDrawForward_);
    guiDrawForward_->Connect("Toggled(Bool_t)", "genfit::EventDisplay", fh,
                             "guiSetDrawParams()");
  }
  frmMain->AddFrame(hf);
  ...
  hf = new TGHorizontalFrame(frmMain);
  {
    ...
    if (drawBackward_)
      guiDrawBackward_->Toggle();
    hf->AddFrame(guiDrawBackward_);
    guiDrawBackward_->Connect("Toggled(Bool_t)", "genfit::EventDisplay", fh,
                              "guiSetDrawParams()");
  }
  frmMain->AddFrame(hf);
  ...
  hf = new TGHorizontalFrame(frmMain);
  {
    ...
    if (drawAutoScale_)
      guiDrawAutoScale_->Toggle();
    hf->AddFrame(guiDrawAutoScale_);
    guiDrawAutoScale_->Connect("Toggled(Bool_t)", "genfit::EventDisplay", fh,
                               "guiSetDrawParams()");
  }
  frmMain->AddFrame(hf);
  ...
  hf = new TGHorizontalFrame(frmMain);
  {
    ...
    if (drawScaleMan_)
      guiDrawScaleMan_->Toggle();
    hf->AddFrame(guiDrawScaleMan_);
    guiDrawScaleMan_->Connect("Toggled(Bool_t)", "genfit::EventDisplay", fh,
                              "guiSetDrawParams()");
  }
  frmMain->AddFrame(hf);
  ...
  hf = new TGHorizontalFrame(frmMain);
  {
    guiErrorScale_ =
        new TGNumberEntry(hf, errorScale_, 6, 999, TGNumberFormat::kNESReal,
                          TGNumberFormat::kNEANonNegative,
                          TGNumberFormat::kNELLimitMinMax, 1.E-4, 1.E5);
    hf->AddFrame(guiErrorScale_);
    guiErrorScale_->Connect("ValueSet(Long_t)", "genfit::EventDisplay", fh,
                            "guiSetDrawParams()");
  ...
  hf = new TGHorizontalFrame(frmMain);
  {
    ...
    if (drawCardinalRep_)
      guiDrawCardinalRep_->Toggle();
    hf->AddFrame(guiDrawCardinalRep_);
    guiDrawCardinalRep_->Connect("Toggled(Bool_t)", "genfit::EventDisplay", fh,
                                 "guiSetDrawParams()");
  }
  frmMain->AddFrame(hf);
  ...
  hf = new TGHorizontalFrame(frmMain);
  {
    ...
    guiRepId_->Connect("ValueSet(Long_t)", "genfit::EventDisplay", fh,
                       "guiSetDrawParams()");
  ...
  hf = new TGHorizontalFrame(frmMain);
  {
    ...
    if (drawAllTracks_)
      guiDrawAllTracks_->Toggle();
    hf->AddFrame(guiDrawAllTracks_);
    guiDrawAllTracks_->Connect("Toggled(Bool_t)", "genfit::EventDisplay", fh,
                               "guiSetDrawParams()");
  }
  frmMain->AddFrame(hf);
  ...
  hf = new TGHorizontalFrame(frmMain);
  {
    ...
    guiTrackId_->Connect("ValueSet(Long_t)", "genfit::EventDisplay", fh,
                         "guiSetDrawParams()");
  ...
  hf = new TGHorizontalFrame(frmMain2);
  {
    ...
    guiEvent2->Connect("ValueSet(Long_t)", "genfit::EventDisplay", fh,
                       "guiGoto2()");
  ...
  hf = new TGHorizontalFrame(frmMain2);
  {
    ...
    guiRefit_->Connect("Toggled(Bool_t)", "genfit::EventDisplay", fh,
                       "guiSetDrawParams()");
  }
  frmMain2->AddFrame(hf);
  ...
  hf = new TGHorizontalFrame(frmMain2);
  {
    ...
    guiDebugLvl_->Connect("ValueSet(Long_t)", "genfit::EventDisplay", fh,
                          "guiSetDrawParams()");
  ...
  hf = new TGHorizontalFrame(frmMain2);
  {
    ...
    guiFitterId_->Connect("Clicked(Int_t)", "genfit::EventDisplay", fh,
                          "guiSelectFitterId(int)");
    hf->AddFrame(guiFitterId_, new TGLayoutHints(kLHintsTop));
    TGRadioButton *fitterId_button =
        new TGRadioButton(guiFitterId_, "Simple Kalman");
  ...
  {
    guiMmHandling_ =
        new TGButtonGroup(hf, "Multiple measurement handling in Kalman:");
    guiMmHandling_->Connect("Clicked(Int_t)", "genfit::EventDisplay", fh,
                            "guiSelectMmHandling(int)");
    hf->AddFrame(guiMmHandling_, new TGLayoutHints(kLHintsTop));
    TGRadioButton *mmHandling_button =
        new TGRadioButton(guiMmHandling_, "weighted average");
    ...
    new TGRadioButton(guiMmHandling_,
                      "weighted, closest to reference for WireMeasurements, "
                      "weighted average else");
    new TGRadioButton(guiMmHandling_,
                      "unweighted, closest to reference for WireMeasurements, "
                      "unweighted average else");
    new TGRadioButton(guiMmHandling_,
                      "weighted, closest to prediction for WireMeasurements, "
                      "weighted average else");
    new TGRadioButton(guiMmHandling_,
                      "unweighted, closest to prediction for WireMeasurements, "
                      "unweighted average else");
  ...
  hf = new TGHorizontalFrame(frmMain2);
  {
    guiSquareRootFormalism_ = new TGCheckButton(
        hf, "Use square root formalism (simple Kalman/simple DAF)");
    if (squareRootFormalism_)
      guiSquareRootFormalism_->Toggle();
    hf->AddFrame(guiSquareRootFormalism_);
    guiSquareRootFormalism_->Connect("Toggled(Bool_t)", "genfit::EventDisplay",
                                     fh, "guiSetDrawParams()");
  }
  frmMain2->AddFrame(hf);
  ...
  hf = new TGHorizontalFrame(frmMain2);
  {
    ...
    guiDPVal_->Connect("ValueSet(Long_t)", "genfit::EventDisplay", fh,
                       "guiSetDrawParams()");
  ...
  hf = new TGHorizontalFrame(frmMain2);
  {
    ...
    guiRelChi2_->Connect("ValueSet(Long_t)", "genfit::EventDisplay", fh,
                         "guiSetDrawParams()");
  ...
  hf = new TGHorizontalFrame(frmMain2);
  {
    ...
    guiDChi2Ref_->Connect("ValueSet(Long_t)", "genfit::EventDisplay", fh,
                          "guiSetDrawParams()");
    lbl = new TGLabel(
        hf, "min chi^2 change for re-calculating reference track (Ref Kalman)");
    hf->AddFrame(lbl);
  }
  frmMain2->AddFrame(hf);
  ...
  hf = new TGHorizontalFrame(frmMain2);
  {
    ...
    guiNMinIter_->Connect("ValueSet(Long_t)", "genfit::EventDisplay", fh,
                          "guiSetDrawParams()");
  ...
  hf = new TGHorizontalFrame(frmMain2);
  {
    ...
    guiNMaxIter_->Connect("ValueSet(Long_t)", "genfit::EventDisplay", fh,
                          "guiSetDrawParams()");
  ...
  hf = new TGHorizontalFrame(frmMain2);
  {
    ...
    guiNMaxFailed_->Connect("ValueSet(Long_t)", "genfit::EventDisplay", fh,
                            "guiSetDrawParams()");
  ...
  hf = new TGHorizontalFrame(frmMain2);
  {
    ...
    guiResort_->Connect("Toggled(Bool_t)", "genfit::EventDisplay", fh,
                        "guiSetDrawParams()");
  }
  frmMain2->AddFrame(hf);
